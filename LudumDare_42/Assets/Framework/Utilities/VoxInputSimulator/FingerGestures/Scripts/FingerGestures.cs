...
public class FingerGestures : MonoBehaviour
{
    ...
    public FGInputProvider trackpadInputProviderPrefab;
    public FGInputProvider inputSimulatorProviderPrefab;
    ...
    void InitInputProvider()
    {
        ...
        if (InputSimulatorCore.isSimulating)
        {
            e.inputProviderPrefab = inputSimulatorProviderPrefab;
        }
        else
        {
            if (IsTouchScreenPlatform(Application.platform))
                ...
                e.inputProviderPrefab = mouseInputProviderPrefab; //trackpadInputProviderPrefab;
        }
    ...
    public void InstallInputProvider(FGInputProvider inputProviderPrefab)
    {
        ...
        if (inputProvider is WorldInputSimulatorProvider)
        {
            InputSimulatorCore.SetWorldInputSimulatorProvider((WorldInputSimulatorProvider)inputProvider);
        }
    ...
    public class Finger
    {
        ...
        public void ForceUpdate(bool p_newDownState, Vector2 p_newPos)
        {
            Update(p_newDownState, p_newPos);
        }
        ...
        internal void Update(bool newDownState, Vector2 newPos)
        {
            ...
            if (filteredOut && !newDownState)
                filteredOut = false;
            //Debug.Log("filteredOut: " + filteredOut);
            //Debug.Log("ShouldProcessTouch: " + !FingerGestures.instance.ShouldProcessTouch( index, newPos ));
            ...
            // process touch
            prevPhase = phase;
            //Debug.Log("newDownState: " + newDownState);
            if (newDownState)
            {
                // new touch? reset finger state
                //Debug.Log("WasDown: " + WasDown);
    ...
    {
        return instance.fingers.Length;
    }
